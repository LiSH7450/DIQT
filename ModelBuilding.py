...
def SVM_permutation_test(X, y):
    'label_permutation'
    df_predict_SVM_permu_Y = pd.DataFrame(y, columns=[0])
    df_permu_Y_SVM = pd.DataFrame(y, columns=[0])
    for r in range(1000):
        ...
        df_predict_SVM_permu_Y = pd.concat([df_predict_SVM_permu_Y, df_predict_temp], axis=1)
        df_permutation_temp = pd.DataFrame(y_permutation, columns=[r])
        df_permu_Y_SVM = pd.concat([df_permu_Y_SVM, df_permutation_temp], axis=1)

    df_predict_SVM_permu_Y.to_csv('df_predict_SVM_permu_Y.csv')
    df_permu_Y_SVM.to_csv('df_permu_Y_SVM.csv')
    ...
    df_predict_SVM_permu_X = pd.DataFrame(y, columns=[0])
    for r in range(1000):
        kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=r)
        model = Pipeline([('ss', StandardScaler()), ('svc',
                                                     SVC(kernel='rbf', gamma=0.001,
        ...
        df_predict_temp = pd.DataFrame(y_predict, columns=[r])
        df_predict_SVM_permu_X = pd.concat([df_predict_SVM_permu_X, df_predict_temp], axis=1)

    df_predict_SVM_permu_X.to_csv('df_predict_SVM_permu_X.csv')
    X_permutation.to_csv('df_permu_X_SVM.csv')

def RF_gridsearch_result(X, y):
    'GridSearch'
    kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=7)
    param_grid = {'n_estimators': range(10, 300, 10), 'max_depth': range(3, 21, 2),
                  'min_samples_split': range(10, 210, 20)}
...
def XGB_gridsearch_result(X, y):
    ...
    for r in range(1000):
        kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=r)
        model = XGBClassifier(eta=grid_search.best_params_['eta'], gamma=grid_search.best_params_['gamma'],
...
def LR_grid_search_result(X, y, pre=0):
    ...
    for r in range(1000):
        kfold = StratifiedKFold(n_splits=5, shuffle=True, random_state=r)
        if pre == 0:
            model = Pipeline([('ss', StandardScaler()), ('LR_2',
                                                         LR(penalty='l2', solver=grid_search.best_params_['LR__solver'],
                                                            C=grid_search.best_params_['LR__C'], max_iter=10000))])
        else:
            model = LR(penalty='l2', solver=grid_search.best_params_['solver'], C=grid_search.best_params_['C'],
                       max_iter=10000)
        model.fit(X, y)
        y_predict = cross_val_predict(model, X, y, cv=kfold)
        df_predict_temp = pd.DataFrame(y_predict, columns=[r])
